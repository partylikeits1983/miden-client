...
export const customAccountComponent = async (): Promise<void> => {
  return await testingPage.evaluate(async () => {
    ...
    console.log("created wallet seed");

    let anchorBlock = await client.getLatestEpochBlock();
    console.log("retrieved latest epoch block");
    ...
    await client.newAccount(accountBuilderResult.account, accountBuilderResult.word, false)
    ...
    console.log("compiled txScript");

    let txIncrementRequest = new window.TransactionRequestBuilder()
      .withCustomScript(txScript)
      .build();
    console.log("created txIncrementRequest");

    let txResult = await client.newTransaction(
      accountBuilderResult.account.id(),
      txIncrementRequest
    );
    console.log("created txResult");
    await client.submitTransaction(txResult);
    console.log("submitted txResult");
  });
};

describe.only("custom account component tests", () => {
